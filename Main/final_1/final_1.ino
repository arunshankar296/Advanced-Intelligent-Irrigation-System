...
namespace Config {
  ...
  const unsigned int PEAK_TIME_START = 1330;    // 6:00 PM (18:00)
  const unsigned int PEAK_TIME_END   = 1400;    // 7:00 PM (19:00)
  const unsigned long MAX_PUMP_RUN_TIME_MS = 120000UL;  // 2 minutes
  ...
  const int WELL_MIN_LEVEL = 0;  // Minimum analog reading to run pump
  ...
  const unsigned int DRIP_MORNING_START = 830;  // 5:00 AM
  const unsigned int DRIP_MORNING_END   = 839;  // 7:30 AM
  const unsigned int DRIP_EVENING_START = 2349; // 4:00 PM
  const unsigned int DRIP_EVENING_END   = 2351; // 6:30 PM
  const float DRIP_LOW_FLOW_THRESHOLD  = 1.0;    // L/min
  const float DRIP_HIGH_FLOW_THRESHOLD = 3.0;    // L/min
  const float DRIP_VOLUME_THRESHOLD    = 1.5;  // liters
...
private:
  ...
  const unsigned long CRITICAL_COOLDOWN_MS = 180000;  // 15 minutes for critical alerts
  const unsigned long STANDARD_COOLDOWN_MS = 3000000; // 1 hour for standard alerts
  const unsigned long INFO_COOLDOWN_MS = 3600000;    // 4 hours for information alerts
